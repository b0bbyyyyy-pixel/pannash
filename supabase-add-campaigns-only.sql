-- Add Campaigns Tables Only
-- Run this if leads and email_connections already exist
...
-- 2. CAMPAIGN LEADS (junction table)
...
DROP POLICY IF EXISTS "Users can view their own campaign leads" ON campaign_leads;
DROP POLICY IF EXISTS "Users can insert their own campaign leads" ON campaign_leads;
DROP POLICY IF EXISTS "Users can update their own campaign leads" ON campaign_leads;
DROP POLICY IF EXISTS "Users can delete their own campaign leads" ON campaign_leads;

DROP POLICY IF EXISTS "Users can view their own email queue" ON email_queue;
DROP POLICY IF EXISTS "Users can insert their own email queue items" ON email_queue;
DROP POLICY IF EXISTS "Users can update their own email queue items" ON email_queue;
DROP POLICY IF EXISTS "Users can delete their own email queue items" ON email_queue;
