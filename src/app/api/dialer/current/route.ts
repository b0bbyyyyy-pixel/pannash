...
/**
 * GET /api/dialer/current?listId=<id>
 * Returns:
...
export async function GET(req: Request) {
  try {
    ...
    const current = await getCurrentLead(supabase, user.id, listId);
    ...
    const queue = await peekQueue(supabase, user.id, current?.id ?? null, 10, listId);
